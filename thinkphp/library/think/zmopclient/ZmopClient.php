<?php
namespace think\ZmopClient;
include('RSAUtil.php');
include('WebUtil.php');
use think\Exception;
use think\zmopclient\lotusphpruntime\logger\Ltlogger;
use think\zmopclient\lotusphpruntime\Lotus;
...
class ZmopClient {
	...
	function __construct($gatewayUrl = '', $appId = '', $charset = 'UTF-8', $privateKeyFilePath, $zhiMaPublicKeyFilePath){
		 ...
		 */
		$lotusHome = dirname(__FILE__) . DIRECTORY_SEPARATOR . "lotusphpruntime" . DIRECTORY_SEPARATOR;
		include($lotusHome . "Lotus.php");
		$lotus = new Lotus();
		$lotus->option["autoload_dir"] = dirname(__FILE__) ;
//		var_dump(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'zmop');
...
//		die;
	...
	public function execute($request) {
		...
		if (isset ($bizRespObject->error_code)) {
			$logger = new LtLogger();
//			$logger = new  \think\zmopclient\lotusphp_runtime\logger\Ltlogger();
	...
	public function decryptAndVerifySign($encryptedResponse, $sign){
		...
		if($success){
			return [1,$decryptedResponse];
		}else{
			return [0,$decryptedResponse];
